...
/**
 * Resource is the class to represent the resource part of URL to access to a web-service or a web-site.
 * It contains directly the resource, parameters of GET request and fragment id.       
 * 
 * @author qween
 *
 */
public class Resource
{
  ...
  * 	@param map -- the container contained the pairs of names and values of the parameters 
  */
  public Resource(String res, Map<String, String> map)
  {
    this.param = map;
    if (!verify(map))
      throw new IllegalArgumentException("Incorrect map of the parameters");
  ...
  public void setMainResource(String res) {
    if (!verify(res, verMRes))
      throw new IllegalArgumentException("Incorrect main resource: '" + res + "'");
    this.mainResource = res;
  }
  
  /**
   * @return the main part of the Resource;<br> 
   *         <Code>null</Code> if the main part of the Resource isn't specified
   */
   ...
   * 	@param res -- the fragment_id string
   */
   ...
   * @param parValue -- the value of parameter
   */
  public void setParam(String parName, String parValue)
  {
    ...
    if (!verify(parValue, verElem))
      throw new IllegalArgumentException("Incorrect parameter value: '" + parName + "'");
  ...
  */
  ...
  public String constructResourceString()
  {
    ...
    res = this.mainResource + res + (!this.fragmentId.equals("") ? "#" + this.fragmentId : "");
    return res;
  }

  public String toString() {
    return "{ Main Resourse: " + this.mainResource + 
      ", Parameters: " + this.param.toString() + 
      ", FragmentId: " + this.fragmentId + 
      "}";
  }
  
  /**
   * 	Parse the resource sting to main part of resource and the set of parameters
   * 
   * 	@param res --- the Resource string
   */
  protected void parseResourceString(String res)
  {
    ...
    if (res_arr.length > 2)
      throw new IllegalArgumentException("The resource string has more than one symbol '#'");
    if (res_arr.length == 2) {
      if (!verify(res_arr[1], "(^$)||((^|.+)(=|\\?|#|\\&)(.+|$))"))
        throw new IllegalArgumentException("Incorrect fragmentId: '" + res_arr[1] + "'");
    ...
    res_arr = res.split("\\?");
    if (res_arr.length == 0) { this.mainResource = ""; return; }
    if (res_arr.length > 2)
      throw new IllegalArgumentException("The resource string hasn't or has more than one symbol '?'.");
    if (!verify(res_arr[0], "(^|.+)(=|\\?|#|\\&)(.+|$)"))
      throw new IllegalArgumentException("Incorect main resource: '" + res_arr[0] + "'");
    this.mainResource = res_arr[0];
    if (res_arr.length == 1) return;

    res_arr = res_arr[1].split("\\&");

    for (int i = 0; i < res_arr.length; i++) {
      String[] par_arr = res_arr[i].split("\\=");
      if ((par_arr.length != 2) || (par_arr[0].isEmpty()) || (par_arr[0].isEmpty()))
        throw new IllegalArgumentException("Incorrect parameter: '" + res_arr[i] + "'");
  ...
  protected static boolean verify(Map<String, String> map) {
    ...
    while(it.hasNext())
    {
      ...
      if ((verify(k, "(^$)||((^|.+)(=|\\?|#|\\&)(.+|$))")) && (verify((String)map.get(k), "(^$)||((^|.+)(=|\\?|#|\\&)(.+|$))")))
      return false;
    }
    return true;
  }
